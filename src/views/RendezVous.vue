<template>
  ...
  <div class="app" style="background-color: #f2f2f2" >
    ...
    <div v-else>
      <br>
      <h4 class="text-center form-title" style="top: 5%">
      Espace Prise de Rendez-Vous
    </h4>
    <br />
    <div class="top">
      ...
      <v-dialog v-model="adddialog" width="900" height="1500">
        ...
        <v-card>
          ...
          <v-card-title> Prendre Votre Rendez-Vous </v-card-title>
...
<br><br>
    ...
    <select v-model="gouvernorat" id="country">
...
<v-col cols="6">
    ...
    <select v-model="municipalite" id="state" :disabled="!gouvernorat">
      ...
      <option v-for="(state, index) in states[gouvernorat]" :key="index" :value="state.code">{{ state.name }}</option>
              ...
              <v-row>
                <v-col cols="12">
                  <v-text-field
                    v-model="servicedemandé"
                    :value="servicedemandé"
                    label="Service Demendé*"
      ...
      <input type="date" id="date" name="date" v-model="date" :rules="[(v) => !!v || 'Champ obligatoire']"   >
    </v-col>
   
    
      <v-col cols="6">
      <label for="heure">Heure :</label>
      <select id="heure" name="heure" v-model="heure">
        <option v-for="(h, index) in heuresDisponibles" :value="h" :key="index">{{ h }}</option>
      </select>
    </v-col>
          ...
          <tr v-for="item in items" :key="item.id" @click="editItem(item)">
            ...
            <td style="text-align: right">
              <v-dialog v-model="editDialog" width="1000" height="1500">
                <template v-slot:activator="{ props }">
                  <v-icon @click="editItem(item)" v-bind="props" color="success"
                    >mdi-pencil</v-icon
                  >
                ...
                <v-card>
                  <v-card-title> Modifier Votre Rendez-Vous </v-card-title>
                  <v-divider :thickness="2"></v-divider>
                ...
                <v-col cols="12">
                  <v-text-field
                    v-model="editedItem.servicedemandé"
                    label="Service Demendé*"
                    :rules="[(v) => !!v || 'Champ obligatoire']"
                    required
                  >
              ...
              <v-row>
                <v-col cols="6">
      <label for="date">Date :</label>
      <input type="date" id="date" name="date" v-model="date" @change="validateSelect" >
    </v-col>
        ...
        <option v-for="(h, index) in heuresDisponibles" :value="h" :key="index">{{ h }}</option>
              ...
              <v-row>
                ...
                <v-col cols="6">
                  ...
                  <v-card-actions>
                    <v-btn @click="editDialog = false" color="primary" type="submit"
                      >Fermer</v-btn
                    >
                  </v-card-actions>
                </v-card>
...
{ 'code': 'Feriana', 'name': 'Feriana' },
          ...
          { 'code': 'Degash', 'name': 'Degash' },
          { 'code': 'El Hamma Jerida', 'name': 'El Hamma Jerida' },
          { 'code': 'Tameghza', 'name': 'Tameghza' },
          { 'code': 'Hazoua', 'name': 'Hazoua' },
]
     }
    };
  ...
  methods: {
    validateSelect() {
      this.selectError = !this.date
    } ,
    async loadHeuresDisponibles() {
      if (!this.date) {
        return;
      }
      try {
        const response = await fetch(
          `https://localhost:7283/api/RendezVous/HeuresDisponibless?date=${this.date}`
        );
        const heuresDisponibles = await response.json();
        console.log(heuresDisponibles);

        this.heuresDisponibles = heuresDisponibles;
        if (heuresDisponibles.length > 0) {
          this.heure = heuresDisponibles[0];
        } else {
          this.heure = null;
        }
      } catch (error) {
  ...
  return new Intl.DateTimeFormat('fr-FR', options).format(date);
    ...
    updateStates() {
      if (this.gouvernorat === '') {
        ...
        this.statesList = this.states[this.gouvernorat];
      }},
  ...
  return dateTime.toLocaleString('fr-FR', options);
    ...
    editItem(item) {
  // Assign the selected item to editedItem
  this.editedItem = { ...item };
  
  // Set editDialog to true to open the dialog
...
,
 ...
 async validate() {
  ...
  async validaten() {
    const { valid } = await this.$refs.forms.validaten();
    return valid;
  },
  submitForm() {
...
async getItems() {
  try {
    ...
    const response = await axios.get(
      "https://localhost:7283/api/RendezVous/ByEmail",
      {
        params: { email: user.email },
      }
    );
    console.log(response.data); // Ajout de cette ligne pour vérifier les données reçues
    // Update the items array to include the FilePath property
    this.items = response.data;
    // Store the items in local storage
    localStorage.getItem("userItems", JSON.stringify(this.items));
  } catch (error) {
    console.error(error);
  }
},
...
async updateItem() {
  try {
    ...
    const response = await axios.put(`https://localhost:7283/api/RendezVous/RendezVous`, {
      // update the properties of the proposition
      id:this.editedItem.id,
      servicedemandé: this.editedItem.servicedemandé,
    ...
    localStorage.setItem("userItems", JSON.stringify(this.items));
   
    this.editDialog = false; // close the dialog
    await this.getItems();
    this.$forceUpdate();
    
    await Swal.fire('Succès', 'La proposition a été modifiée avec succès!', 'success');
    ...
    console.error(error);
    this.dialog = false;
    await Swal.fire('Error', 'Une erreur est survenue lors de la modification de la proposition', 'error');
  }
},
    ...
    async supprimerItem(id) {
      try {
        ...
        if (result.isConfirmed) {
          ...
          await axios.delete(`https://localhost:7283/api/RendezVous/${id}`);
          // Remove the item from the items array
          this.items = this.items.filter((item) => item.id !== id);
          // Show success alert
          await Swal.fire({
            icon: "success",
            title: "Rendez-Vous supprimée avec succès!",
        ...
        Swal.fire({
          icon: "error",
          title: "Erreur",
          text: "Une erreur est survenue lors de la suppression de votre Rendez-Vous. Veuillez réessayer plus tard.",
    ...
    async fetchAvailableTimes() {
        if (!this.date) return;
        const response = await axios.get('https://localhost:7283/api/RendezVous/availableTimes/${this.date}');
  ...
  try {
    ...
    if (valid) {
      ...
      const nouvelItem = {
        ...
        servicedemandé: this.servicedemandé,
        heure:this.heure,
        date: this.date,
        motif: this.motif,
        autres: this.autres,
      ...
      const response = await axios.post(
        "https://localhost:7283/api/RendezVous/RendezVous",
        nouvelItem
      );
      ...
      await Swal.fire({
        icon: "success",
        title: "Rendez-Vous créée avec succès!",
        showConfirmButton: false,
        timer: 1500,
      });
      // Mettre à jour la liste des éléments à partir du stockage local
      this.items = JSON.parse(localStorage.getItem("userItems")) || [];
      // Forcer la mise à jour du composant
      this.$forceUpdate();
    }
  } catch (error) {
