<template>
  ...
  <div class="app" style="background-color: #f2f2f2">
    ...
    <div v-else>
      <br>
    <h4 class="text-center form-title" style="top: 4%">Espace Propositions</h4>
    <div class="top">
      <v-dialog v-model="adddialog" width="900" height="1500">
        ...
        <v-card>
          <v-card-title > Ajouter Votre Proposition </v-card-title>
          <v-divider :thickness="2" ></v-divider>

          <v-sheet width="600" style="margin-left: 100px">
            <v-form ref="form">
              <p class="p3">  <v-icon style="size: 50px;" >mdi-account</v-icon>   Informations Personnelles :</p>
              <br />
              <v-row>
                <v-col cols="12">
                  <v-text-field
              ...
              <p class="p3"> <v-icon style="size: 50px;" >mdi-pencil</v-icon> Informations concernant la proposition :</p>
              <br />
              <v-row>
                  <v-col cols="6">
...
<v-col cols="6">
    ...
    <select v-model="municipalite" id="state" :disabled="!gouvernorat"  :rules="[(v) => !!v || 'Champ obligatoire']">
              ...
              <v-row>
                ...
                <v-col cols="6">
                  <v-textarea
                    :value="avantages"
                    v-model="avantages"
                    label="Avantages pour la Communité*"
                    counter
                    maxlength="120"
                    required
                    clearable
      ...
      <p class="p1">
        Vous avez au total
        <b style="color: #2196f3">{{ nombreDeLignes }}</b> Proposition(s)
      </p>
    </div>
    <div v-if="loading" class="spinner">
    ...
    <div class="table-wrapper">
      ...
      >
        ...
        <tbody>
              ...
              <v-dialog v-model="editDialog" width="1000" height="1500">
                <template v-slot:activator="{ props }">
                  <v-icon @click="editItem(item)" v-bind="props" color="success"
                    >mdi-pencil</v-icon
                  >
                  <v-icon @click="supprimerItem(item.id)" color="error">mdi-delete</v-icon>
                </template>
                <v-card>
          <v-card-title > Modifier Votre Proposition </v-card-title>
          <v-divider :thickness="2" ></v-divider>
          ...
          <v-card-actions>
            <v-btn @click="editDialog = false" color="primary">Fermer</v-btn>
          </v-card-actions>
        </v-card>
              </v-dialog>
            </td>
...
export default {
  ...
  data() {
    return {
      ...
      file: "",
      codePostalRules: [
        (v) => !!v || "veuillez entrez un CodePostal",
        (v) =>
          /^[0-9]{4}$/.test(v) ||
          "Le CodePostal doit contenir exactement 4 chiffres",
      ],
...
async getItems() {
  try {
    ...
    const response = await axios.get(
      "https://localhost:7283/api/Proposition/ByEmail",
      {
...
async updateItem() {
  try {
    const response = await axios.put(`https://localhost:7283/api/Proposition/Modification`, {
      // update the properties of the proposition
      id:this.editedItem.id,
      code: this.editedItem.codePostal,
        objet: this.editedItem.objet,
        description: this.editedItem.description,
        avantages: this.editedItem.avantages,
        cout: this.editedItem.cout,
    ...
    console.error(error);
    this.dialog = false;
    await Swal.fire('Erreur', 'Une erreur est survenue lors de la modification de la proposition', 'error');
  }
},
  ...
  try {
    ...
    if (result.isConfirmed) {
      ...
      await axios.delete(`https://localhost:7283/api/Proposition/${id}`);
    ...
    Swal.fire({
      icon: 'error',
      title: 'Erreur',
      text: 'Une erreur est survenue lors de la suppression de l\'item. Veuillez réessayer plus tard.',
      ...
      try {
        ...
        if (valid) {
          ...
          const response = await axios.post(
            "https://localhost:7283/api/Proposition/propositions",
            nouvelItem
          );
          ...
          await Swal.fire({
            icon: "success",
            title: "Proposition créée avec succès!",
            showConfirmButton: false,
            timer: 1500,
          });
    ...
    this.$forceUpdate();
        ...
        Swal.fire({
          icon: "error",
          title: "Erreur",
          text: "Une erreur est survenue lors de la création de la proposition. Veuillez réessayer plus tard.",
          confirmButtonText: "OK",
        });
      }
    },
  ...
  mounted() {
    ...
    if (!userData) {
  // Redirect to the login page if user data is not found in local storage
  window.location.href = "/login";
} else {
  // Parse the user data and assign it to the `user` variable
